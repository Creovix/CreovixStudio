...
const DOT = "inline-block h-2.5 w-2.5 min-h-2.5 min-w-2.5 shrink-0 rounded-full";

export function HubPlatformDot({ id, title }: { id: PlatformId; title?: string | undefined }) {
  if (id === "TIKTOK") {
    return (
      <span className="inline-block h-2.5 w-2.5 shrink-0 rounded-full bg-cyan-400" title={title} aria-hidden />
    );
  }
